...
begin
  F1 := 1 * power(x, 3) + 1 * power(x, 2) + (4) * x + 6;
end;
...
begin
  ...
  textout(10, 10, 'Найти площади фигуры, ограниченной кривой 1*x^3+(1)*x^2+(4)*x+(6) и осью Ох (в положительной части по оси Оу)');
...
begin
  ...
  begin
    ...
    begin
      setpencolor(clnavy);
      e := x;
      Rectangle(x0 + round(s1 * mx), round(y0 - round(F1(p/2+e) * my) * 1.4), x0 + round(s1 * mx + (k + l)), y0);
    end;
    if (x0 + round(s1 * mx + l)) = (x0 + round(x * mx)) then
    begin
      setpencolor(clnavy);
      e := x;
      for var v := x0 + round(s1 * mx) to x0 + round(s2 * mx) do
        Rectangle(x0 + round(s1 * mx + l), round(y0 - round(F1(p/2+e) * my) * 1.4), x0 + round(s1 * mx + (k + l)), y0);
      l := l + k;
      e := e + l;
      inc(w);
      if (w + 1) = ras then
...
begin
  var f2: real;
  f2 := 1 / 4 * power(x, 4) + 1 / 3 * power(x, 3) + 2 * power(x, 2) + 6 * x;
  func1 := f2;
end;
...
begin
  ...
  var n: integer;
  Textcolor(5);
  Writeln('Вычисление площади фигуры, ограниченной кривой 1*x^3+(1)*x^2+(4)*x+(6) и осью Ох (в положительной части по оси Оу)');
  Textcolor(3);
  ...
  writeln('Может увеличить размер графика?');
  casem := casemf(casem);
  case casem of
    1: grapx(a, b, h, n);
    0:
